...
We will to be vectorizing the lists into a binary matrix, aka a Tensor. 
...
One Hot Encoding means conveting the dataset to a vector of 0's and 1's where the items in the list index are 1 at the appropriate location.
...
We've encoded the images, but also need to categorically encode the labels so the model knows how to bucket things. We will be using the `categorical_crossentropy()` loss function (discussed in the next section). This method is dictated to us as part of our image recognition recipe. For more information, look at the help entry for `r ?to_categorical`.
...
`layer_dense()` takes the following parameters:

 * units - dimensions of the output
 * activation - The tensor operation to be applied in the layer transformation
 * input_shape - dimensions of the input (excluding the samples axis)
...
See page 153 in Deep Learning with R for more examples of how to visualize convnet layers.
...
## Tensorflow Tools for R by RStudio

 * [Tensorflow Tools for R by RStudio](https://tensorflow.rstudio.com/)
 * [Tensorflow + R Examples](https://tensorflow.rstudio.com/gallery/)
 ...
 * [DataCamp](https://www.datacamp.com/courses/machine-learning-with-tree-based-models-in-r)
